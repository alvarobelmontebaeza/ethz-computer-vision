...
class Simulator(object):
	...
	def getMeasurementModel(self, locations):
		...
		"""
		# Obtain measurement model for each door independently
		...
		# Compute measurement model as the complementary of NO seeing any door. 
	...
	def moveParticles(self, particles, uk):
		...
		# Initialize prior_particles array
		prior_particles = particles
		N = self.param.num_particles

		if self.param.circular_hallway:
			# Consider a circular hallway
			for i in range(N):
				# Add gaussian noise to movement
				motion = uk + np.random.normal(0.0,self.param.odometry_stdev)
				# Translate particles, taking into account the circular hallway
				prior_particles[i] = np.mod((particles[i] + motion),self.param.domain+1)
		else:
			# Consider a Lineal hallway
			for i in range(N):
				# Add gaussian noise to movement
				motion = uk + np.random.normal(0.0,self.param.odometry_stdev)

				# Check if we are out of bounds. If so, change direction of motion
				if (particles[i] + motion) > self.param.domain: # Right Boundary
					prior_particles[i] = self.param.domain - np.mod(particles[i] + motion, self.param.domain)
				elif (particles[i] + motion) < 0.0: # Left Boundary
					prior_particles[i] = 0.0 + np.abs(particles[i] + motion)
				else: # Normal translation of particles
					prior_particles[i] = particles[i] + motion
	...
	def measureParticles(self, prior_particles, zk):
		...
		# Obtain measurement model at the locations described in the particles
		prob_door = self.getMeasurementModel(prior_particles)
		particle_weights = np.zeros(self.param.num_particles)

		# Consider the appropriate measurements depending on the sensor output
		if zk == 1:
			particle_weights = prob_door
		else:
			particle_weights = (1 - prob_door)
	...
	def resampleParticles(self, prior_particles, particle_weights):
		...
		######## Implementation of the 'Resampling wheel' algorithm ########

		# Initialize parameters
		N = self.param.num_particles
		maxW = max(particle_weights)
		index = np.random.randint(0,N)
		beta = 0.0		
		resampled_particles = np.zeros(len(prior_particles))

		# Obtain the same amount of particles after resampling
		for i in range(N):
			beta += np.random.uniform(0.0,2.0 * maxW)
			while beta > particle_weights[index]:
				beta -= particle_weights[index]
				index = (index + 1) % N
			
			resampled_particles[i] = prior_particles[index]
